...
def initialize_model(model):
    ...
    modelsList = [os.path.join('./models/', each) for each in os.listdir('./models/') if 'u2net' in each]  # we list the model files
    if not len(modelsList):
        ...
        epochNum = int(modelFile.split('/')[-1].split('_')[-1].split('.')[0])+1
